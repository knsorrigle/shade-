    ...
    private var effectControls: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(model.fogAvailable
                 ...
                 : "Needs the scene depth buffer, which only injection can reach. Launch a game injected to use it.")
                .font(.caption)
                .foregroundStyle(model.fogAvailable ? Color.secondary : Color.orange)
                .fixedSize(horizontal: false, vertical: true)
